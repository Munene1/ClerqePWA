...
export default function Sidebar(props: {
  ...
  return (
    <aside
      ref={sidebarRef}
      className={`fixed left-0 top-0 z-50 flex h-full w-full flex-col overflow-hidden border-r border-gray-200/40 bg-white transition-[transform] duration-300 ease-out transform-gpu dark:border-gray-800/40 dark:bg-black md:w-80 pt-[var(--sat,0px)] md:pt-0 md:z-auto ${
        props.open ? "translate-x-0" : "-translate-x-full"
      } md:translate-x-0`}
    >
      <div className="sticky top-0 z-10 flex items-center justify-between border-b border-gray-200/40 px-3 py-2.5 md:py-3 dark:border-gray-800/40">
        <ClerqeLogo className="h-10 text-gray-700 dark:text-gray-300" />
        <button
          onClick={props.onClose}
          className="rounded-[3px] p-1 text-gray-500 hover:bg-gray-100 md:hidden dark:text-gray-400 dark:hover:bg-gray-900"
        >
        ...
        >
          <div className="pb-40">
            <div className="px-2 pt-2 pb-1">
              ...
              <button
                onClick={() => { navigate("/people"); props.onClose(); }}
                className="mt-0.5 flex w-full items-center gap-2.5 rounded-[3px] px-3 py-2 text-sm font-medium text-gray-700 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-900"
              >
            ...
            <div className="px-2 pb-4">
              ...
              {props.sessions.map((s) => {
                const text = s.last_message?.content || "Session " + s.id.slice(0, 8);
                return (
                  <button
                    key={s.id}
                    onClick={() => { navigate("/sessions/" + s.id); props.onClose(); }}
                    className="flex w-full items-center gap-2.5 rounded-[3px] px-3 py-2 text-left transition-colors hover:bg-gray-100 dark:hover:bg-gray-900"
                  >
                    <Icon name="chat_bubble" className="shrink-0 text-base text-gray-400" />
