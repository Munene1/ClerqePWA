...
export default function App() {
  ...
  useEffect(() => {
    if (isLoggedIn && socket.connectionState === "connected" && sessionData.sessions.length === 0 && !sessionData.sessionsLoading) {
      sessionData.loadSessions(() => socket.listSessions());
    }
  }, [isLoggedIn, socket.connectionState, sessionData.sessions.length, sessionData.sessionsLoading]);
  ...
  const isBeneficiariesRoute = location.pathname.startsWith("/beneficiaries") || location.pathname.startsWith("/people");
...
{isLoggedIn && (
        <div className={isBeneficiariesRoute ? "hidden md:block" : ""}>
          <Sidebar
            open={sidebarOpen}
            theme={theme}
            userName={userName}
            ...
            onLoadSessions={() => sessionData.loadSessions(() => socket.listSessions())}
            onClose={() => { if (window.innerWidth < 768) setSidebarOpen(false); }}
            onLogout={handleLogout}
            onSetTheme={(t) => setTheme(t)}
          />
        </div>
      )}
      {isLoggedIn && !isBeneficiariesRoute && (
        <div
          className="fixed left-1.5 z-30 flex items-center gap-1 rounded-full px-1 py-0.5 backdrop-blur-md md:hidden"
          style={{ top: "calc(0.25rem + var(--sat, 0px))" }}
        >
          <button
            onClick={() => setSidebarOpen(true)}
            className="inline-flex h-9 w-9 items-center justify-center rounded-full bg-gray-200/90 text-gray-500 hover:bg-gray-300 active:bg-gray-400 dark:bg-gray-800 dark:text-gray-400 dark:hover:bg-gray-700 dark:active:bg-gray-600"
      ...
      <div className={isLoggedIn ? "md:ml-80" : ""}>
    <Routes>
      <Route path="/beneficiaries" element={
        isLoggedIn ? (
          <BeneficiaryListPage accessToken={sessionState.session?.access_token ?? ""} />
        ) : <Navigate to="/" replace />
      } />
      <Route path="/beneficiaries/:id" element={
        isLoggedIn ? (
          <BeneficiaryDetailPage accessToken={sessionState.session?.access_token ?? ""} />
        ) : <Navigate to="/" replace />
      } />
      <Route path="/people" element={
        isLoggedIn ? (
          <PeoplePage accessToken={sessionState.session?.access_token ?? ""} />
        ) : <Navigate to="/" replace />
      } />
